...
** フィルタとして指定できるのは，現在の値（<code>madoka::UInt64</code>）を受け取り，適用後の値（<code>madoka::UInt64</code>）を戻す関数へのポインタです．
** 関数オブジェクトを渡すことも可能ですが，引数と戻り値の型を <code>madoka::UInt64</code> にする必要があります．
** フィルタが <var>max_value</var> を超える値を戻したときは自動的に <var>max_value</var> へと補正するため，<code>UINT64_MAX</code> を超えなければ問題にはなりません．
