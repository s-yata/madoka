...
h2. Get information of a sketch (to be written)
...
** <code>filter()</code> applies a filter to a sketch, or more precisely, <code>filter()</code> applies a filter to all the values in a sketch. The 1st argument specifies the filter.
...
** A function object is also acceptable but the type of its argument and return value must be <code>madoka::UInt64</code>.
...
h2. Use huge pages (to be written)
...
h2. Specify a seed (to be written)
